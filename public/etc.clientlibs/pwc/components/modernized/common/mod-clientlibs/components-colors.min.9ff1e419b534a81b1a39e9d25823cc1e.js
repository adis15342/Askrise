...
PwC.components.collectionColors.clientlibs = {
    ...
    loadMoreScrollable : function(path, link, componentId,currentPagePath){
        ...
        $.ajax(path+ '.dynamic.html?filter='+filter+'&currentPagePath='+currentPagePath+'&'+ link).done(function(data) {
            ...
            var $items = $(data).find('.collection__item');
            ...
            var loadMoreButton = $(data).find('#loadmore-'+componentId).html();
            if(!loadMoreButton){
                if($("#loadmore-"+componentId).is(':visible')){
                    $("#loadmore-"+componentId).toggle()
                }
            } else {
                $("#loadmore-"+componentId).html($(data).find('#loadmore-'+componentId).html());
                if(!$("#loadmore-"+componentId).is(':visible')){
                    $("#loadmore-"+componentId).toggle()
                }
            }
...
function initializeIsotope(componentId,selectedTag,nexLink,createdDate,isDeepLink,moreResultsLink,currentPagePath){
    ...
    if(!moreResultsLink && !nexLink && !(typeof componentId === 'function')) {
        $("#moreLinks-"+componentId).toggleClass("collection__more-links");
    }
    /* Helper */
    $.extend({
        ...
        $(this).on('change', 'select', function(){
            ...
            $.ajax(path+ '.dynamic.html?filter='+value+'&currentPagePath='+currentPagePath+'&start=0').done(function(data) {
                ...
                if(!loadMoreButton){
                    ...
                    if(!$("#moreLinks-"+componentId).hasClass("collection__more-links")){
                        $("#moreLinks-"+componentId).toggleClass("collection__more-links");
                    }
                }
            });
...
PwC.components.contactscollection.listeners = {
    ...
    paginateListAjax : function (path, link, nodeName,deepLinkId,isDeepLink){
        ...
        $.ajax(path + ".dynamic.html?" + link + filter).done(function(data) {
            $("#listId-inner-" + nodeName).html($(data).eq(0).find("#listId-inner-" + nodeName).html());
            $("#listId-inner-" + nodeName).append($(data).eq(1));
        });
    ...
    filterListValues : function (filterId, componentId,deepLinkId,isDeepLink){
        ...
        $.ajax(path + "?" + filter).done(function(data) {
            $("#listId-outer-" + componentId).html($(data).eq(0).html());
	    $("#listId-outer-" + componentId)[0].appendChild($(data)[1]);
        });
...
var secondaryNavTwoCol = (function () {
	...
	function init() {
		...
		$('.secnav-twocol .popover-trigger').on('click', function(e) {
			...
			$('.secnav-twocol__popover-image img').on("load", function() {
...
// object responsable for handling highlights modal behavior
{
    ...
    // if so, shows the read more button
    {
        ...
        if (window_width > self.CONST.MAX_SIZE_MOBILE)
        {
            ...
            read_more       = titleHeight + paragraph.height() > modal.height();
        }
        else
        {
...
var progressIndicator = (function() {
					...
					if (w.innerWidth >= 992) {
						...
						if(localStorage.getItem("animateFlag") == 1){
							...
							menuListArrayItems.forEach(function(value, key) {
								...
								}, (200 * key)) //animation time of each dots
							});
	...
	function init(config) {
		let delay = componentAnimationDelayFlag ? 3000 : 1500;
...
if (typeof(bottomKickParent) !== 'undefined' && bottomKickParent !== null) {
    ...
    // Commenting this line of code for now to verify PR-15044
...
var slimnav2 = (function() {
    ...
    var _handlers = function() {
        ...
        function closeNavSearch(e) {
            ...
            $('#slim-hamburger').removeClass('open').attr('aria-label', function() {
                return $(this).attr('data-open-aria-label');
            });
            _closeSlimNav();
            _navItemLimit = _resize.navItemLim();
        }
        ...
        //Search Button
        $('.nav-search > button, .mobile-search-btn').on('click keydown', function (event) {
        ...
        // ------------------------ Mobile Navigation Accessibilty ----------------------------

        if (windowWidth() < 992) {
            $('.show-subnav .slimheader-close').on('click touchend keydown', function (e) { //sub-level nav close icon
                if (e.type === 'click' || e.type === 'touchend' || (e.type === 'keydown' && e.key === 'Enter')) {
                    $('.cmp-container, .slim-navigation, #skipNav a, #logoUrl').removeClass('hidden');
                    $('#slim-hamburger').focus(); //focus on the hamburger icon
                }
            });

            $('#slim-hamburger').on('click touchend keydown', function (e) { //hamburger icon
                if (e.type === 'click' || e.type === 'touchend' || (e.type === 'keydown' && e.key === 'Enter')) {
                    e.stopPropagation();
                    e.preventDefault();
                    $('.slim-navigation').scrollTop(0);
                    if ($(this).hasClass('open')) {
                        ...
                        _closeSlimNav();
                        $('.cmp-container, #skipNav a').removeClass('hidden');
                        $(this).attr('aria-label', $(this).attr('data-open-aria-label'));
                    } else {
                        $('#skipNav a').addClass('hidden');
                        $('.cmp-container').addClass('hidden'); //hiding all the containers
                        $('.slimnav-v2-breadcrumbs a, .slimnav-v2-bc-toggle').attr('tabindex', '-1');
                        $('.slim-navigation nav a').attr('tabindex', '0');
                        $('.slim-navigation .nav-secondary a').attr('tabindex', '0');
                        $('.root.container > .cmp-container:first-child, #header .cmp-container').removeClass('hidden'); //showing only the overlay container

                        $(this).addClass('open');
                        $(this).attr('aria-label',$(this).attr('data-close-aria-label'));
                        var calcHeight = $(window).outerHeight() - $('.slimheader-v2').outerHeight();
            ...
            $('.mobile-search-btn').on('click touchend keydown', function (e) { //search icon
                if (e.type === 'click' || e.type === 'touchend' || (e.type === 'keydown' && e.key === 'Enter')) {
                    $('#skipNav a').addClass('hidden');
                    $('.cmp-container').addClass('hidden'); //hiding all the containers
                    $('.slimnav-v2-breadcrumbs a, .slimnav-v2-bc-toggle').attr('tabindex', '-1');
            ...
            $('#close-search').on('click touchend keydown', function (e) { //close search icon
                if (e.type === 'click' || e.type === 'touchend' || (e.type === 'keydown' && e.key === 'Enter')) {
                    $('.cmp-container, #skipNav a').removeClass('hidden');
                    $('#slim-hamburger').focus(); //focus on the hamburger icon
                }
            ...
            var lastLevel1ActiveLink;
            $('.slim-navigation nav a').on('click touchend keydown', function (e) { //nav-primary links
                if (e.type === 'click' || e.type === 'touchend' || (e.type === 'keydown' && e.key === 'Enter')) {
                    lastLevel1ActiveLink = this;
                    if ($(this).attr('href') === '#') {
                        setTimeout(function () {
                            $('.slim-navigation, #skipNav a, #logoUrl').addClass('hidden');
                        }, 500); //adding timeout to ensure the transition effect
                    }
                    setTimeout(function() {
                        var eleId = $('.show-subnav').attr('id');
                        $('#' + eleId).find('a.lv2-label')[0].focus();
                    }, 500);
                }
            });

            $('.slimnav-mobile-header > p').on('click touchend keydown', function (e) { //sub-level nav back button link
                if (e.type === 'click' || e.type === 'touchend' || (e.type === 'keydown' && e.key === 'Enter')) {
                    $('.slim-navigation, #skipNav, #logoUrl').removeClass('hidden');
                    $(lastLevel1ActiveLink).focus();
                }
            });
        }
...
function adjustStatChars() {
    ...
    $('.statbar-5050-wrapper').each(function (index, element) {
        ...
        var statLeftWidth = Math.round($(statLeft).width() || 0)
...
PwC.components.peopleContactscollection.listeners = {
    peoplePaginateListAjax: function (path, link, container) {
        ...
        $.ajax(path + ".dynamic.html?" + link).done(function (data) {
            var $newItems = $($(data).find(".people-contact-collection-cards-container"));
...
function onHoverOfCard(element, mouseleave) {
...
function equalizeHeightsExpertCollection(cards) {
    const allHaveNoThumbnail = cards.every(card =>
        card.querySelector(".people-contact-collection-card-no-thubmail")
    );
    cards.forEach(card =>{
         card.style?.removeProperty("height")
         calculateItemHeight(card)
    });
    let maxHeight = Math.max(...cards.map(card => {
        const haveThumbnail = card.className.includes("people-contact-collection-card-reduced-thubmail") && window.innerWidth > 767;
        const peopleContactInfoDiv = card?.querySelector('.people-contact-collection-info');
        // Get computed styles of the div
        const computedStyle = getComputedStyle(peopleContactInfoDiv);
        // Extract padding values
        const paddingTop = parseFloat(computedStyle.paddingTop);
        const paddingBottom = parseFloat(computedStyle.paddingBottom);
        if (allHaveNoThumbnail) {
            return card?.querySelector(".people-contact-collection-bio")?.getBoundingClientRect().height + paddingTop + paddingBottom;
        } else if (haveThumbnail){
            return card?.querySelector('.people-contact-collection-image ').getBoundingClientRect()?.height + card?.querySelector(".people-contact-collection-bio")?.getBoundingClientRect().height + paddingTop + paddingBottom;
        }
        return card?.getBoundingClientRect().height;
    }));
    if (allHaveNoThumbnail) {
        const maxHeightSoical = Math.max(...cards.map(card => {
            let peopleContactSocailHeight = card.querySelector(".people-contact-collection-card-no-thubmail")?.querySelector(".people-contact-collection-link-container-border")?.getBoundingClientRect().height || 0;
            if (window.innerWidth < 768) {
                peopleContactSocailHeight += 15;
            }
            if (!card.style.height) {
                return peopleContactSocailHeight;
            }
            return 0
        }));
        maxHeight += maxHeightSoical
    }
    cards.forEach(card => card.style.height = `${maxHeight}px`);
    cards.forEach(card => {calculateBioHeight(card)});
  
}

// Calulate the social icon container height and set the height of the container  
function equalizeHeightsOfSoical(cards, expertPeopleCollectionCard) {
    const cardArray = Array.isArray(cards) ? cards : Array.from(cards);
    if(expertPeopleCollectionCard){
        cards.forEach(card => {
            card.querySelector(".people-contact-collection-link-container-border")?.style?.removeProperty("height")
            card.querySelector(".people-contact-collection-info")?.style?.removeProperty("--defaultheight")
        });
    }
    ...
    const maxHeightBio = Math.max(...cardArray.map(card => {
        const element = card?.querySelector('.people-contact-collection-bio');
        return element ? element?.getBoundingClientRect().height: 0;
    }));
    ...
    cardArray.forEach(card => {
        ...
        if (expertPeopleCollectionCard && window.innerWidth > 767) {
            const peopleContactInfoDiv = card?.querySelector('.people-contact-collection-info');
            const computedStyle = getComputedStyle(peopleContactInfoDiv);
            const haveThumbnail = card.className.includes("people-contact-collection-card-reduced-thubmail");
            // Extract padding values
            const paddingTop = parseFloat(computedStyle.paddingTop) || 0;
            const paddingBottom = parseFloat(computedStyle.paddingBottom) || 0;
            if (peopleContactInfoDiv && !haveThumbnail) {
                peopleContactInfoDiv.style.setProperty("--defaultheight", `${card?.querySelector('.people-contact-collection-bio').getBoundingClientRect()?.height + paddingTop + paddingBottom}px`)
            }

            else if (haveThumbnail) {
                const collectionBioDiv = card?.querySelector('.people-contact-collection-bio');
                const collectionContactDiv = card?.querySelector(".people-contact-collection-link-container-border");
                const collectionImage = card?.querySelector('.people-contact-collection-image ')
                collectionBioDiv.style.setProperty("--defaultheightbio", `${maxHeightBio}px`)
                peopleContactInfoDiv.style.setProperty("--defaultheight", `${maxHeightBio + paddingTop + paddingBottom + (collectionContactDiv?.getBoundingClientRect().height || 0)}px`)
                peopleContactInfoDiv?.style?.setProperty("--defaulttop", `${collectionImage.getBoundingClientRect()?.height}px`)
                peopleContactInfoDiv?.style?.setProperty("--onhoverTop", `${collectionImage.getBoundingClientRect()?.height - (collectionContactDiv?.getBoundingClientRect().height || 0)}px`)
            }
        }
    });
}

const setSocialIconWidth = (cards) => {
    const cardArray = Array.isArray(cards) ? cards : Array.from(cards);
    cardArray.forEach(card => {
        const mediaIconDivWidth = card?.querySelector('.people-contact-collection-media-social')?.getBoundingClientRect().width || 0;
        const contactinfo = card?.querySelector('.expertpeoplecollection__contactinfo-container');
        if (contactinfo) {
            contactinfo.style.setProperty("--social-icon-width", `${mediaIconDivWidth ? mediaIconDivWidth + 15 : mediaIconDivWidth}px`)
        }
    })
}

// init function for contact collection all functions are defined in inside init function
function init(container, addClass) {
    const peopleContactCard = container.querySelectorAll('.people-contact-collection-cards-container');
    const expertPeopleCollectionCard = container?.querySelectorAll('.expertpeoplecollection__card-container')?.length ? true : false;
    ...
    function mainEqualize() {
        if (window.innerWidth > 767 && !expertPeopleCollectionCard) {
        ...
        if(expertPeopleCollectionCard && addClass){
            if (peopleContactCard.length == 1) {
                peopleContactCard[0].classList.remove(
                    "col-lg-3",
                    "col-sm-6",
                    "col-md-6"
                );
                peopleContactCard[0].className += " expert-people-collection__single-card-container col-lg-6 col-md-10 col-sm-10";
            } else {
                peopleContactCard[0].classList.remove(
                    "expert-people-collection__single-card-container",
                    "col-lg-6",
                    "col-md-10",
                    "col-sm-10"
                );
                peopleContactCard[0].classList.add(
                    "col-lg-3",
                    "col-sm-6",
                    "col-md-6"
                );
        ...
        for (let i = 0; i < peopleContactCard.length; i++) {
            ...
            if ((window.innerWidth < 768) && chunkSize === 1) {
                chunkSize = 0;
                chunkIndex++;
            } else if ((window.innerWidth > 767 && window.innerWidth < 1200) && ((expertPeopleCollectionCard && chunkSize === 2) || (!expertPeopleCollectionCard && chunkSize === 3))) {
                chunkSize = 0;
                chunkIndex++;
            } else if (chunkSize === 4) {
                chunkSize = 0;
        ...
        chunks.forEach(function (elem) {
            const cards = elem;
            if(peopleContactCard.length == 1){
                setSocialIconWidth(cards)
            } else {
                if (!expertPeopleCollectionCard) {
                    equalizeTags(cards);
                }
                setSocialIconWidth(cards)
                equalizeHeightsOfSoical(cards, expertPeopleCollectionCard);
                if (expertPeopleCollectionCard) {
                    equalizeHeightsExpertCollection(cards);
                } else {
...
const calculateBioHeight = (element) => {
    if (element?.querySelector('.people-contact-collection-short-bio')) {
        const peopleContactCollectionShortBio = element?.querySelector('.people-contact-collection-short-bio');
        const expertPeopleCollectionCardNoThumbnail = element.className.includes("people-contact-collection-card-no-thubmail") || false;
        if (element.type !== "mouseleave") {
            if (!( expertPeopleCollectionCardNoThumbnail)) {
                const peopleContactCollectioninfoHeight = element?.querySelector('.people-contact-collection-info')?.getBoundingClientRect().height || 0;
                const peopleContactCollectionShortBioHeight = peopleContactCollectionShortBio?.getBoundingClientRect().height || 0;
                const peopleContactCollectionBottomContainerHeight = element?.querySelector('.people-component-collection-bottom-container')?.getBoundingClientRect().height || 0;
                let calDescriptionHeight = (peopleContactCollectioninfoHeight - peopleContactCollectionShortBioHeight) + peopleContactCollectionBottomContainerHeight;
                calDescriptionHeight = element?.getBoundingClientRect().height - calDescriptionHeight;
                if(calDescriptionHeight < 0) {
                    Math.abs(calDescriptionHeight)
                }
                var peopleContactCollectionShortBioEle = peopleContactCollectionShortBio.querySelector("p");
                var lineHeight = parseInt(window.getComputedStyle(peopleContactCollectionShortBioEle).getPropertyValue("line-Height"), 10);
                var lines = calDescriptionHeight / lineHeight;
                peopleContactCollectionShortBioEle.style.setProperty("--line-clamp-value", `${Math.round(lines)}`)
            }
        }
    }
}

const calculateItemHeight = (element) => {
    if (window.innerWidth > 767) {
        const contactInfoDiv = element?.querySelector('.people-contact-collection-info');
        const isReducedCard = element?.querySelector(".expertpeoplecollection__card--reduced") ? true : false;
        if (contactInfoDiv) {
            defaultCardHeight = contactInfoDiv.style["height"];
            if (isReducedCard) {
                const bottomContainerHeight = element?.querySelector('.people-contact-collection-link-container-border')?.getBoundingClientRect().height || 0;
                const contactInfoDivHeight = contactInfoDiv.getBoundingClientRect().height || 0;
                const contactImageHeight = element?.querySelector('.people-contact-collection-image')?.getBoundingClientRect().height || 0;
                const totalBottomContentHeight = contactInfoDivHeight + bottomContainerHeight;
                const availableBottomSpacing = (element.getBoundingClientRect().height - contactImageHeight) - contactInfoDivHeight;

                if (totalBottomContentHeight > contactImageHeight || availableBottomSpacing < bottomContainerHeight) {
                    contactInfoDiv.classList.remove("expertpeoplecollection__info--animation-disabled");
                    contactInfoDiv.style.setProperty("--height", `${totalBottomContentHeight}px`)
                } else {
                    contactInfoDiv.classList.add("expertpeoplecollection__info--animation-disabled");
                    const remainingHeightForImage = element.getBoundingClientRect().height - contactImageHeight;
                    contactInfoDiv.style.setProperty("--height", `${remainingHeightForImage}px`)
                }
            } else {
                contactInfoDiv.style.setProperty("--height", `${element?.getBoundingClientRect().height}px`)
            }
        }
    }
...
$(document).ready(function () {
    ...
    peopleComponentsList?.forEach(function (mainElment) {
        ...
        init(mainElment, true);
        mainElment.querySelector(".people-contact-collection-cards-container-reduced")?.classList?.add("people-contact-collection-cards-container-reduced-visible");
        $(window).resize(function () {
                init(mainElment , false);
        });
    })
});
